...
class SignalProgressRequest(BaseModel):
    phase: str = Field(..., description="Current workflow phase: 'sdg', 'training', or 'eval'")
    step_id: str = Field(
        ..., description="Stable identifier for this step (snake_case, e.g. 'check_models')"
    )
    label: str = Field(..., description="Human-readable description of the step")
    status: str = Field(
        "active", description="Step status: 'active' for in-progress, 'completed' for done"
    )
...
@router.post(
    ...
    summary="Report the current workflow step to the chat UI for dynamic progress display",
)
