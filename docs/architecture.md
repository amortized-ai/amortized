...
Morty is defined by its prompts and skills in `agents/`, not by a deployed service — the agent runtime was removed from the k8s manifests. What remains is an orchestrator plus per-workflow subagent prompts:

- **Orchestrator** (`agents/orchestrator/`): `identity.md` (persona, guardrails), `workflow.md` (delegation logic)
- **Subagent workflows** (`agents/sdg/`, `agents/training/`): `workflow.md` per subagent, plus `skills/` directories with on-demand guides and reference payloads

`make prompt` generates the prompt configs from the agents directory. Any MCP-capable agent runtime can load these and connect to the Amortized server's `/mcp` endpoint to operate the platform.
...
| Agent behaviour | `agents/orchestrator/workflow.md`, `agents/sdg/`, `agents/training/` |
