...
| Recall | ~40% | ~80%+ |
| F1 | ~45% | ~82%+ |
| Judge pass rate | ~55% | ~90%+ |

## Customization

- **More data**: Increase `num_samples` in `synth.yaml` to 500-1000 for better extraction quality
- **Different model**: Change `model_name_or_path` in `train.yaml` (try `Qwen/Qwen3-4B` for higher accuracy)
- **Different entity types**: Edit the `possible_values` under `entity_type` in `synth.yaml`
- **Different text domains**: Edit the `possible_values` under `text_type` in `synth.yaml`
- **Longer texts**: Increase `max_length` in `train.yaml` (and adjust batch size down)
