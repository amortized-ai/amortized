...
async def test_list_models_with_gateway() -> None:
    fake_models = [
        GatewayModel(name="chat", provider="openai", endpoint_type="llm/v1/chat"),
        GatewayModel(name="claude-haiku", provider="anthropic", endpoint_type="llm/v1/chat"),
    ]
    ...
    assert data["models"][0]["endpoint_type"] == "llm/v1/chat"
...
async def test_list_models_caches_result() -> None:
    ...
    async def mock_fetch() -> list[GatewayModel]:
        nonlocal call_count
        call_count += 1
        return [GatewayModel(name="chat", provider="openai", endpoint_type="llm/v1/chat")]
...
async def test_fetch_gateway_models_caches() -> None:
    ...
    models_mod._models_cache = [
        GatewayModel(name="cached", provider="test", endpoint_type="llm/v1/chat")
    ]
