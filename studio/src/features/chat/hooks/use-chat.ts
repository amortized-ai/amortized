...
export function useChat() {
    ...
    if (_activeRequests.has(convId)) {
      const pollId = setInterval(() => {
        ...
        if (!_activeRequests.has(convId)) {
          ...
          stopThinkingRef.current = null
          setMessages(restoreMessages(getConversationMessages, convId))
          const restored = getConversationMessages(convId)
          const last = restored[restored.length - 1]
          setChatState(last?.role === "assistant" && !last.content ? "idle" : "done")
        }
      }, 500)
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      const lockId = currentConversationId ?? useChatStore.getState().currentConversationId ?? ""
      if (chatState === "streaming" || _sendLock.has(lockId)) return
