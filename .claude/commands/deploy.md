...
```bash
ssh shiv@REDACTED_IP 'cd /home/shiv/{user}/amortized && git fetch origin && git reset --hard origin/{branch} && make refresh-{user}'
```
...
```bash
ssh shiv@REDACTED_IP 'kubectl --context kind-amortized -n amortized-{user} get pods -o custom-columns="NAME:.metadata.name,STATUS:.status.phase,STARTED:.status.startTime"'
```
